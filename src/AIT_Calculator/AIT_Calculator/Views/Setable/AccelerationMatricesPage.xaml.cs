...
{
    /// <summary>
    /// Логика взаимодействия для AccelerationMatricesPage.xaml
    /// </summary>
    public partial class AccelerationMatricesPage : UserControl
    {
        public CarDataModel CarDataModel;
        public AccelerationMatricesPage(CarDataModel carDataModel)
        {
        ...
        private void SelectCSVFileButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                ...
                // Используем ваш существующий метод парсинга
                var (axData, ayData) = CSVParsingService.ParseCSV(openFileDialog.FileName);

                // Обновляем данные в CarDataModel
                CarDataModel.UpdateAccelerationData(axData, ayData, axData, ayData);
            }
        }
        private void SelectXLSXFileButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                FilePathTextBlock.Text = openFileDialog.FileName;
                // Используем ваш существующий метод парсинга
                var (axData, ayData) = ExcelParsingService.ParseExcel(openFileDialog.FileName);
